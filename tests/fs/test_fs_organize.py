...
from reny.fstools.builders.fsprms import FSEntryParamsBase, FSEntryParamsOrganize
...
class FSOrganizeTests(FSTest):
    ...
    def test_organize_by_type(self):
        ...
        organized_dir = os.path.join(self.src_dir, 'png')
        self.assertTrue(os.path.exists(organized_dir), 
                      "Expected png directory was not created")
        ...
        self.assertTrue(len(files_in_organized) > 0, 
                      "No files found in organized png directory")
    ...
    def test_organize_filtered_files(self):
        ...
        image_dir = os.path.join(self.src_dir, 'png')
        self.assertTrue(os.path.exists(image_dir), 
    ...
    def test_organize_recursive(self):
        ...
        image_dir = os.path.join(self.src_dir, 'png')
        self.assertTrue(os.path.exists(image_dir), 
                      "Recursive organization did not create png directory")
    ...
    def test_print_organized_view_mixed_file_types(self):
        ...
        # The test data contains PNG files, which should all be categorized as 'png' type
        ...
        # Should contain at least one directory (png directory for PNG files)
        dir_lines = [line for line in output_lines if '|->/' in line]
        self.assertGreater(len(dir_lines), 0, "Should have at least one organized directory")
        
        # Should contain png directory since test data has PNG files
        png_dir_found = any('png' in line.lower() for line in dir_lines)
        self.assertTrue(png_dir_found, "Should contain png directory for PNG files")
